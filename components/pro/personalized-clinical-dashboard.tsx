...
export function PersonalizedClinicalDashboard() {
  ...
  const widgets: DashboardWidget[] = [
    ...
    },
    {
      ...
      defaultPosition: 2,
      colSpan: 'lg:col-span-1',
      content: <p className="text-muted-foreground text-sm p-4 text-center">Secure messaging interface placeholder.</p>
    },
    {
      id: 'scheduleOverview',
      ...
      defaultPosition: 3,
      colSpan: 'lg:col-span-1',
      content: <p className="text-muted-foreground text-sm p-4 text-center">Today's appointments and on-call responsibilities.</p>
    }
  ];
  ...
  const renderContent = () => {
    ...
    if (error) {
        return <div className="p-4"><Card className="bg-destructive/10 border-destructive"><CardContent className="p-4 text-center text-destructive-foreground"><ServerCrash className="mx-auto h-8 w-8 mb-2"/><p>Failed to load dashboard data.</p></CardContent></Card></div>
    }
    if (!dashboardData) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            return (
                ...
                <Card className={cn("h-full border-none shadow-none", widget.isFrequentlyUsed && !isEditMode && "bg-transparent")}>
                    ...
                    {isEditMode && <GripVertical className="h-5 w-5 text-muted-foreground cursor-grab" title="Drag to reorder" />}
                    </CardHeader>
