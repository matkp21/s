...
import { SolvedQuestionPapersViewerComponent } from '@/components/medico/solved-question-papers-viewer';
...
export default function MockPYQsPage() {
  ...
  const { mutate: runGenerateExam, data: examData, isPending: isLoading, error, reset } = useAiAgent<MedicoExamPaperInput, MedicoExamPaperOutput>(generateExamPaper, MedicoExamPaperOutputSchema, {
  ...
  const onSubmit: SubmitHandler<ExamPaperFormValues> = async (data) => {
    // Casting is necessary because the form includes `examFormat` which is not in MedicoExamPaperInput
    const agentInput: MedicoExamPaperInput = {
      examType: data.examType,
      year: data.year,
      count: data.count
    };
    await runGenerateExam(agentInput);
  };
