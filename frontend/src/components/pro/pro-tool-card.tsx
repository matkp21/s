...
import type { ProTool, ActiveToolId } from './pro-dashboard';

interface ProToolCardProps {
  tool: ProTool;
  onLaunch: (toolId: ActiveToolId) => void;
  isFrequentlyUsed?: boolean;
}

const ProToolCardComponent: React.FC<ProToolCardProps> = ({ tool, onLaunch, isFrequentlyUsed }) => {
  const cardContent = (
    <motion.div
      ...
      onClick={() => !tool.href && onLaunch(tool.id)}
      role="button"
      tabIndex={0}
...
export const ProToolCard = React.memo(ProToolCardComponent);
